...
class BaseConvLayer_new(Layer):
    ...
    **kwargs
    ...
    def __init__(self, incoming, num_filters, filter_size, stride=1, pad=0,
                 ...
                 nonlinearity=nonlinearities.rectify, flip_filters=True,f= init.Constant(0.),
                 n=None, **kwargs):
        super(BaseConvLayer_new, self).__init__(incoming, **kwargs)
        ...
        self.f = self.add_param(f, self.get_W_shape(), name='f', trainable = False, regularizable=False)
        if b is None:
            self.b = None
        else:
...
class Conv2DLayer_new(BaseConvLayer_new):
    """
    lasagne.layers.Conv2DLayer(incoming, num_filters, filter_size,
    stride=(1, 1), pad=0, untie_biases=False,
    W=lasagne.init.GlorotUniform(), b=lasagne.init.Constant(0.),
    ...
    **kwargs
    ...
    def __init__(self, incoming, num_filters, filter_size, stride=(1, 1),
                 ...
                 nonlinearity=nonlinearities.rectify, flip_filters=True,
                 convolution=T.nnet.conv2d, **kwargs):
        super(Conv2DLayer_new, self).__init__(incoming, num_filters, filter_size,
                                          stride, pad, untie_biases, W, b,
                                          nonlinearity, flip_filters, n=2,
                                          **kwargs)
