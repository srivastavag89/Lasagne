...
class DenseLayer_new(Layer):
    ...
    def __init__(self, incoming, num_units, W=init.GlorotUniform(), 
                 b=init.Constant(0.), nonlinearity=nonlinearities.rectify,
                 num_leading_axes=1,f = init.Constant(0.), **kwargs):
        super(DenseLayer_new, self).__init__(incoming, **kwargs)
        self.nonlinearity = (nonlinearities.identity if nonlinearity is None
                             else nonlinearity)
        ...
        ########added here
        self.f = self.add_param(f, (num_inputs, num_units), name='f', trainable = False, regularizable=False)
        #self.wb = self.add_param(wb, (num_inputs, num_units), name='wb', trainable = False, regularizable=False)
        #
        if b is None:
            self.b = None
        else:
